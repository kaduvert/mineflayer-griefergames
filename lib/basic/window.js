...
module.exports = function inject(bot, options) {
    ...
    bot.window.loadPatterns = (apiDataObj, patternHead) => {
        ...
        Object.keys(windowPatterns).forEach(windowPatternName => {
            bot.window.patterns[patternHead + bot.apiData.patternHeadNameSeparator + windowPatternName] = windowPatterns[windowPatternName]
        })
    }

    bot.window.matchesItemPattern = (patternHead, patternName, stack) => {
        const { title: titleRegex, lore: loreRegex } = bot.resolveItemPattern(patternHead, patternName)
    ...
    bot.window.getItemPatternMatches = (patternHead, patternName, stack) => {
        const { title: titleRegex, lore: loreRegex } = bot.resolveItemPattern(patternHead, patternName)
        const stackName = stack.customName
        const stackLore = stack.customLore
        const matches = {
    ...
    bot.window.getMatchingItem = (patternHead, patternName, window) => {
        const pattern = bot.resolveItemPattern(patternHead, patternName)
