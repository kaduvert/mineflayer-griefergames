...
module.exports = function inject(bot, options) {
	...
	bot.chat.resolveCommand = (patternHeads, patternName) => {
		if (typeof patternHeads === 'string') {
			patternHeads = [patternHeads]
		}
		const command = patternHeads.find(resolver = bot.ggData[resolver]?.commands?.[patternName])
		return command ?? patternName
	}

	bot.chat.sendFallibe = (options) => {
		const {
			patternHeads = [],
			patternHead,
			command
		...
		const msg = command instanceof Array ? bot.chat.buildCommand(bot.chat.resolveCommand(patternHeads, command[0]), ...command.splice(1)) : command
