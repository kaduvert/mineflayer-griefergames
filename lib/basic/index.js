...
module.exports = function inject(bot, options) {
    ...
    bot.resolveEventPattern = (heads, identifier, emitter) => {
        ...
        for (const head of heads) {
            const { chatPatterns, windowPatterns } = bot.apiData[head]
            ...
            if (eventOrigin) {
                return `${eventOrigin}:${head}${bot.apiData.patternHeadNameSeparator}${identifier}`
            }
        }
        return identifier
    }

    bot.resolveItemPattern = (head, identifier) => bot.apiData[head].itemPatterns[identifier]
    ...
    bot.loadChatPatterns = (apiDataObj, patternHead = '') => {
        const chatPatterns = apiDataObj.chatPatterns
        ...
        Object.keys(chatPatterns).forEach(chatPatternName => {
            ...
            bot.addChatPatternSet(patternHead + bot.apiData.patternHeadNameSeparator + chatPatternName, chatPattern, { repeat: true, parse: true })
        })
    }


    bot.loadPatterns = (apiDataObj, patternHead) => {
        bot.loadChatPatterns(apiDataObj, patternHead)
        bot.window.loadPatterns(apiDataObj, patternHead)
    }

    bot.loadPatternsAndGetData = (apiDataObjName) => {
        const apiDataObj = bot.apiData[apiDataObjName]
        bot.loadPatterns(apiDataObj, apiDataObjName)
        return apiDataObj
    }
