module.exports = function inject(bot, options) {
    ...
    bot.plot.info.parseId = (str) => str.split(plot.info.idSeparator).map(e => +e)
    ...
    bot.plot.info.isEqualId = (one, two) => one[0] === two[0] && one[1] === two[1]
    ...
    bot.on('chat:plot->playerEntered', (stringId, username) => {
        bot.emit('misc:plot->playerEntered', bot.plot.info.parseId(stringId), username)
    })

    bot.on('chat:plot->playerLeft', (stringId, username) => {
        bot.emit('misc:plot->playerLeft', bot.plot.info.parseId(stringId), username)
    })
}
