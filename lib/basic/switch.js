...
module.exports = function inject(bot, options) {
	...
	bot.switch.getServerInformation = (server) => {
	...
	bot.switch.canJoin = (server, rank = bot.playerUtils.getRank()) => {
		const { totalSlots, occupiedSlots, onlineStatus } = bot.switch.getServerInformation(server)
	...
	bot.switch.navigate = async () => {
		...
		if (bot.serverInfo.isHub()) {
			...
			bot.switch.toServer(targetServer)
