...
module.exports = function inject(bot, options) {
	...
	bot.switch.getPortalArmorStands = (maxDistance = 3) => {
		return Object.values(bot.entities).filter(e => e.objectType === 'Armor Stand' && !!e.metadata?.[2] && (e.position.distanceTo(bot.entity.position) < 3)).reduce((acc, curr) => [...acc, { nameTag: curr.metadata[2], position: curr.position }], [])
	}

	bot.switch.getJoinableFromBotPosition = () => {
		const armorStands = bot.switch.getPortalArmorStands()
		let isJoinable = true
