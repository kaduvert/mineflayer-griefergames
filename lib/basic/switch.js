...
module.exports = function inject(bot, options) {
	...
	bot.switch.isJoinableFromHologramDisplay = (position) => {
		const serverInfo = bot.pattern.hologram.findMatching(switcher.hologramPatterns.serverStatus, position)
		...
		if (onlineStatusMatch[0] !== 'Online') isJoinable = false

		const [onlinePlayers, maxPlayers] = onlinePlayerCountMatch.map(e => +e)
	...
	bot.switch.navigate = async () => {
		...
		if (bot.serverInfo.isHub()) {
			...
			while (!bot.switch.isJoinableFromHologramDisplay(closestAbsoluteStableBlock)) await bot.delay(10)
