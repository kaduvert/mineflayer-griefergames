...
module.exports = function inject(bot, options) {
    ...
    bot.spawner.getLootableStacks = (window) => {
        return window.containerItems().filter(stack =>
            ...
            !bot.item.matchesPattern('spawner', 'availableExperience', stack) &&
            !bot.item.matchesPattern('spawner', 'nextUpdate', stack) &&
            !bot.item.matchesPattern('spawner', 'settings', stack)
        )
    }
