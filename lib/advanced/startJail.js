module.exports = function inject(bot, options) {
    ...
    bot.startJail.startTokenPurchase = (window) => {
        return bot.window.clickFallible({
            window,
            slot: bot.item.findMatchingIn(window, 'startJail', 'purchaseToken').slot,
            patternHead: 'startJail',
    ...
    bot.startJail.confirmTokenPurchase = (window) => {
        return bot.window.clickFallible({
            window,
            slot: bot.item.findMatchingIn(window, 'startJail', 'confirmPurchase').slot,
            patternHead: 'startJail',
    ...
    bot.startJail.cancelTokenPurchase = (window) => {
        return bot.window.clickFallible({
            window,
            slot: bot.item.findMatchingIn(window, 'startJail', 'cancelPurchase').slot,
            patternHead: 'startJail',
